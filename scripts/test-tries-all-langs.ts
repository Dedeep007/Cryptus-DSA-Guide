...
const TOPIC_FILE = 'tries.json';

async function testTriesTopic() {
                ...
                if (allPassed) {
                    ...
                    results.filter(r => !r.passed).forEach(r => {
                        if (lang === 'cpp') {
                            ...
                            if (r.compileOutput) console.log('      CMP OUT:', r.compileOutput.substring(0, 200));
                            console.log('      ACTUAL:', r.actual ? r.actual.substring(0, 200) : 'null');
                        }
                    });
                    totalFailed++;
                }
...
testTriesTopic().catch(console.error);
